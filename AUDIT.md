...
> **2026-08-25** — `8488263` **P1** : inventaire complet des appelants de `PpmService.list()` (11 sites) et `ActualiteService.list()` (1 site) sur `polissage` à jour. Un seul écran converti — `features/prmp/ppm-marches.ts` (« PPM & marchés rattachés ») appelle désormais `listePage(page, 15)`. Les 10 autres appels PPM sont des lectures **référentielles** (résolution d'un libellé/d'une FK, jointure en mémoire pour un calendrier ou une recherche par référence sur l'ensemble du périmètre) qui ont besoin du jeu complet — les paginer casserait la jointure. `features/prmp/mes-ppm-marches.ts` (« Mes PPM & marchés ») écarté malgré un rendu direct : garde-fou d'affichage client (`statut du dossier ≠ BROUILLON`) redondant avec le filtrage serveur documenté (§3396 `api-endpoints.md`) mais non garanti stable dans le temps — à revisiter avec un filtre serveur explicite plutôt que de se fier à la redondance actuelle. `features/admin/actualites-admin.ts` (seul écran `ActualiteService.list()`, hors `mesActualites()` qui est un autre endpoint déjà scopé) écarté : ses deux onglets « En cours » / « Historique » sont deux filtres mémoire sur `statut` affichés **simultanément** (compteurs des deux onglets visibles en permanence), et l'API `/api/actualites` n'expose aucun paramètre `statut` — paginer d'abord donnerait des compteurs faux ; il faudrait un paramètre serveur, chantier séparé. **État P1** : 2 écrans sur ~3 candidats éligibles identifiés à ce jour (`marches-list.ts`, `ppm-marches.ts`) ; `dossiers` non entamé (attend les paramètres de filtre backend, chantier suivant). Suite front 113 → 116/116 (`ppm-marches.spec.ts` réécrit : 5 tests pagination + les 2 tests d'état d'erreur P9 déjà en place, contre 2 avant).
> **2026-08-25** — **P9 clos** sur les 3 derniers écrans écartés à chaque tranche précédente (les tableaux de bord à tuiles) : `features/pilotage/kpi-dashboard.ts`, `features/pilotage/rapports-page.ts`, `features/secretaire/secretaire-dashboard.ts`. Le motif « une requête → une liste → `<app-etat-erreur>` en pleine page » ne se transposait pas tel quel : un tableau de bord agrège plusieurs sources indépendantes, et remplacer tout l'écran dès qu'UNE tuile sur plusieurs échoue détruit les données des autres — tandis qu'afficher une tuile à 0 déguise l'échec réseau en résultat réel, exactement le défaut que P9 corrige ailleurs. Traitement différencié selon le nombre réel de sources : `kpi-dashboard.ts` n'en a qu'**une** (`GET /api/kpis/tableau-bord` sert les 3 sections) — un échec y prive réellement tout l'écran de données, donc le motif plein-écran existant est correct, appliqué tel quel. `secretaire-dashboard.ts` en a **deux**, combinées jusqu'ici par un `forkJoin` qui les faisait tomber ensemble ; séparées en deux souscriptions indépendantes, chacune avec son propre signal d'erreur. L'état d'erreur est posé **une seule fois** dans `shared/dashboard/dashboard-shell.ts` (seul composant de coquille de tableau de bord du dépôt) : `WorklistItem.error` / `KpiTile.error` par tuile, `pipelineError` pour la section pipeline (réutilise `<app-etat-erreur>`), un unique événement `reessayer` que le tableau de bord consommateur résout en ne rejouant que la ou les sources effectivement en échec. `rapports-page.ts` **écarté, à raison** : ce n'est pas un tableau de bord — deux boutons de téléchargement sans aucune donnée affichée à l'écran ; l'échec y est déjà couvert par le toast de l'intercepteur global et le bouton redevient cliquable (`busy` retombe), sans risque de zéro trompeur puisqu'il n'y a rien à afficher. Suite front 125 → 133/133 (+3 fichiers : `kpi-dashboard.spec.ts`, `dashboard-shell.spec.ts` — le mécanisme partagé, verrouillé une fois —, `secretaire-dashboard.spec.ts` — contrat observable des deux sources indépendantes).
