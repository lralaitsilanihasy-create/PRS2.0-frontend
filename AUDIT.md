...
>
> **2026-08-27 — les 124 avertissements ESLint d'accessibilité sont soldés** (`6523668` → `1d097e5`, 8 commits, 34 fichiers). Décompte de départ : 72 `click-events-have-key-events`, 37 `interactive-supports-focus`, 15 `label-has-associated-control` ; à l'arrivée **0**, et les trois règles reprennent la sévérité « error » — les tolérer en avertissement est ce qui avait permis l'accumulation.
> **Le diagnostic de départ était à revoir** : ces avertissements ne décrivaient pas 124 éléments à rendre focalisables, mais **un seul motif recopié dans 30 fichiers**. Le voile d'une modale portait `(click)="fermer()"`, et le dialogue un `(click)="$event.stopPropagation()"` dont l'unique raison d'être était de neutraliser le premier. Le correctif « mécanique » qu'appelle l'intitulé des règles — poser `tabindex`/`role`/`keydown` sur le voile — aurait **dégradé** l'accessibilité : une tabulation sans nom accessible, ajoutée à l'intérieur d'un piège de focus. Les deux gestionnaires ont donc été supprimés et le geste déplacé dans la directive `appModale` (entrée `appModaleClicExterieur`), qui écoute le voile et teste `composedPath()`. Le voile redevient un décor ; **Échap est l'équivalent clavier** du clic sur le voile, et l'était déjà.
