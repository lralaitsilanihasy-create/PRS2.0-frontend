...
| **Précondition de circuit non remplie** | l'étape précédente n'est pas atteinte | `dispatch` d'un dossier non `PRET_DISPATCH` ou **doublon** de dispatch ; `examen` d'un dossier non `PRET_DISPATCH` ; `vérification` d'un PV non `SIGNE` |
...
```json
{
  "pipelineParStatut": { "PRET_DISPATCH": 5, "EN_EXAMEN": 21, "CLOTURE": 47, "RETIRE": 2 },
...
## Vérifications
**Ressource** `/api/verifications` — POST/PUT : profil `VERIFICATEUR` (titulaire ou délégué) ; DELETE : `ADMINISTRATEUR`. Écriture limitée à sa localité (dossier hors localité → 403, sauf Président). Lecture filtrée par localité.

> **Précondition de circuit (création) → 409** : le PV référencé (`idPv`) doit être au statut **`SIGNE`** (§3.6, « travaille uniquement sur PV au statut SIGNE »).

> **Effet `[Auto]`** : si `obsLevees = true`, le dossier passe au statut `CLOTURE` et le Chargé de publication est notifié (clôture éligible).

**Champs `VerificationDto`**

| Champ (JSON) | Type | Obligatoire | Contraintes |
|---|---|---|---|
| idVerification | number | Oui (PK, au POST) | clé primaire |
| idReception | number | Oui | @NotNull |
| idPv | number | Oui | @NotNull |
| imCtrlVerif | string | Non | max 7 |
| dateVerif | string (date) | Non | |
| observation | string | Non | max 500 |
| obsLevees | boolean | Non | si `true` → dossier `CLOTURE` |
...
| POST | /api/verifications | `VerificationDto` | `VerificationDto` | 201, 400, 403 | VERIFICATEUR (titulaire/délégué) |
| PUT | /api/verifications/{id} | `VerificationDto` | `VerificationDto` | 200, 400, 403, 404 | VERIFICATEUR (titulaire/délégué) |
