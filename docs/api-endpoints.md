...
**Ressource** `/api/inscriptions` — Instruction des inscriptions **PRMP et UGPM** (§3.1). Consultation et écriture réservées à l'**Administrateur** ; le **téléchargement d'une pièce** est ouvert à l'Administrateur **ou** au propriétaire de l'inscription. `GET /en-attente` liste les deux types (champ **`type`** ∈ `PRMP`/`UGPM` sur `InscriptionEnAttenteDto`) : une UGPM a `idPrmpTutelle` renseigné et `entitesDeclarees` vide (pas d'entités propres). `POST /{login}/valider` d'une **UGPM** active directement le compte (aucune entité à instruire — corps `ValidationInscriptionRequest` inutile) ; `refuser` fonctionne à l'identique.
